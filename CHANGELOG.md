...
4. **Caller identity is captured on every sampled request.** The root span and each generation carry the request IP (`client.address` / `langfuse.*.metadata.client_ip`), the llmapi account (`langfuse.user.id` = email, else `user:<id>`), User-Agent, and the provider key label used for that hop. IP/UA still honor `REQUEST_ANALYTICS_LOG_CLIENT=false`.
