...
import { deleteSettingByKey, insertSettingForUsers, readSettingValue } from '../migrate/settings-compat.js';

const SETTING_KEY = 'enable_mcp';

export function up(db: Db): void {
  if (readSettingValue(db, SETTING_KEY) != null) return;

  const row = db.prepare('SELECT COUNT(*) AS count FROM api_keys').get() as { count: number };
  const isExistingInstall = row.count > 0;

  insertSettingForUsers(db, SETTING_KEY, isExistingInstall ? '1' : '0');
}

export function down(db: Db): void {
  deleteSettingByKey(db, SETTING_KEY);
}
