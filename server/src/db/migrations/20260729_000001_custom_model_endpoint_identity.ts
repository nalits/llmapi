...
function hasUserId(db: Db): boolean {
  const cols = db.prepare('PRAGMA table_info(models)').all() as Array<{ name: string }>;
  return cols.some(c => c.name === 'user_id');
}

function userIdExtra(db: Db): string {
  return hasUserId(db) ? ',\n      user_id INTEGER REFERENCES users(id) ON DELETE CASCADE' : '';
}

function carried(db: Db): string {
  return hasUserId(db) ? `${CARRIED_COLUMNS}, user_id` : CARRIED_COLUMNS;
}
...
export function down(db: Db): void {
  ...
  rebuildModels(db, userIdExtra(db), carried(db), 'UNIQUE(platform, model_id)');
}
