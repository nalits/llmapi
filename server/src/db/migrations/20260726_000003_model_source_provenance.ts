...
import { readSettingValue } from '../migrate/settings-compat.js';
...
export function up(db: Db): void {
  ...
  // stands.
  try {
    const catalogJson = readSettingValue(db, 'catalog_applied_json');
    if (!catalogJson) return;
    const parsed = JSON.parse(catalogJson) as { models?: unknown };
