...
import { getDb, resolveUserFromApiKey } from '../db/index.js';
...
export type ResolvedAuth =
  | { kind: 'unified'; userId: number; systemPrompt: null }
  | { kind: 'profile'; userId: number; profileId: number; name: string; systemPrompt: string | null };
...
/**
 * Resolve an inference credential. Unified keys live in user_api_keys
 * (both `llmapi-` and legacy `freellmapi-` prefixes). A client-profile
 * key carries its own prompt and binds the owning user.
 */
export function resolveAuth(token: string | undefined): ResolvedAuth | null {
  if (!token) return null;
  const unified = resolveUserFromApiKey(token);
  if (unified && timingSafeStringEqual(token, unified.key)) {
    return { kind: 'unified', userId: unified.userId, systemPrompt: null };
  }
  if (!token.startsWith(CLIENT_PROFILE_KEY_PREFIX)) return null;
  const row = getDb().prepare(
    'SELECT id, user_id, name, system_prompt, enabled FROM client_profiles WHERE token_hash = ?',
  ...
  const prompt = row.system_prompt != null && row.system_prompt.trim().length > 0
    ? row.system_prompt
    : null;
  return { kind: 'profile', userId: row.user_id, profileId: row.id, name: row.name, systemPrompt: prompt };
}
